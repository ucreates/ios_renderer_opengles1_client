...
#import "CircleBehaviour1.h"
#import "iOSObjectiveCFoundation.h"
#import "iOSObjectiveCGLES1Renderer.h"
@interface CircleViewController1 ()
@property GLES1Renderer* renderer;
@property NSMutableArray<CircleBehaviour1*>* behaviours;
@end
...
- (void)viewDidLoad {
    ...
    self->behaviours = [[NSMutableArray<CircleBehaviour1*> alloc] init];
    for (int i = 0; i < 1; i++) {
        CircleBehaviour1* behaviour = [[CircleBehaviour1 alloc] init];
...
- (void)viewDidDisappear:(BOOL)animated {
    ...
    for (CircleBehaviour1* behaviour in self->behaviours) {
        [behaviour onDestroy];
    }
